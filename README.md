...
Все модули, классы и функции в этом репозитории содержат полные строки документации (docstrings) на русском языке, которые описывают их назначение, параметры и возвращаемые значения.

### Основные модули

-   `src/rlcourse/sem01/algorithms.py`: Реализации базовых алгоритмов обучения с подкреплением, таких как метод Монте-Карло, TD(0) и Q-обучение.
-   `src/rlcourse/sem01/discretization.py`: Утилиты для дискретизации непрерывного пространства состояний в среде CartPole.
-   `src/rlcourse/sem01/envs.py`: Фабричные функции для создания сред `gymnasium` (CartPole, FrozenLake).
-   `src/rlcourse/sem01/policies.py`: Реализации общих политик, таких как ε-greedy и softmax.
-   `src/rlcourse/sem01/video.py`: Утилиты для записи видео с демонстрацией работы агента в среде.

Для получения подробных инструкций по использованию скриптов и инструментов, пожалуйста, обратитесь к строкам документации в файлах, расположенных в директориях `sem01_mdp_bellman/scripts/` и `sem01_mdp_bellman/tools/`.
